...
- Using shared library : LD_LIBRARY_PATH="/home/ravi/MEGA/programs/c/sorber/libraries;$LD_LIBRARY_PATH" ./runtime_librarytest "Today is not the day"
...
- Be careful that the values you pass to the functions declared in `ctype.h` are inside the range of an `unsigned char` or `EOF`.
- `int isalnum(int c)` : Returns nonzero if c is an alphanumeric character (letter or digit)
- `int isalpha(int c)` : Returns nonzero if c is a letter
- `int iscntrl(int c)` : Returns nonzero if c is a control character
- `int isdigit(int c)` : returns nonzero if c is a decimal digit
- `int isgraph(int c)` : returns nonzero if c is a any printing character except space.
- `int islower(int c)` : returns nonzero if c is a lowercase letter
- `int isprint(int c)` : returns nonzero if c is printing character including space
- `int ispunct(int c)` : returns nonzero if c is any printing character other than a letter, a digit or space.
- `int isspace(int c)` : returns nonzero if c is a whitespace character
- `int isuppper(int c)`
- `int isxdigit(int c)`: returns nonzero if c is a hexadecimal digit
- `int tolower(int c)` : if c an uppercase letter, returns the corresponding lowercase letter. OW, returns c unchanged.
- `int toupper(int c)` : if c a lowercase letter, returns the corresponding uppercase letter. OW, returns c unchanged.

##### Standard definitions

- `sttdef.h` standard type/macro definitions
...
- `EDOM         // domain error`
- `ERANGE       // range error`
- Integer constants representing certain error conditions
- `errno` Integer variable that contains an error number or 0 for no error.

##### `assert.h` (diagnostics)

- `void assert(int expresssion)`
- if `NDEBUG` macro is defined when `assert.h` is included, does nothing. OW, checks whether the expression is true.
...
- `ptrdiff_t` signed type fo the difference between 2 pointers
...
- `jmp_buf` usd for non-local jumps

##### Possible format specifiers

- `%d` **signed int** decimal representation (equivalent to `%i`)
- `%u` **unsigned int** decimal representation
- `%x` **unsigned int** lowercase hexadecimal representation
- `%X` **unsigned int** uppercase hexadecimal representation
...
- `%g` **float/double** notation `%f` or `%e` chosen depending on value
- `%c` character (passed as **int**), text representation
- `%s` string
- `%p` pointer
- `%n` number of characters written upto now will be written to **int** that the corresponding argument points to.

- Can change the type of the printed variable by inserting one of the following characters between the % sign and the type character
- `h` for d, i, o, u, x, X: **short int** instead of **int** <br> for n: store result in **short int** instead of **int**
- `l` for d, i, o, u, x, X: **long int** instead of **int** <br> for n: store result in **long int** instead of **int**
- `L` for e, E, f, F, g, G: **long doubles** instead of **float/double**

- There are some flags and modfiers that can be put between the % and the type character
- `-` : left alignment, pad on the right with spaces
- `+` : print plus sign if positive
- space : print space if positive
- `0` : pad with zeros instead of spaces (for numbers only)
...
- Every global variable is defined as static automatically.
- Is it possible to have an automatic global variable? No. Because global variables are by default static.

### structures

- One can pass a whole structure as a parameter to a function, or return one as a value from a function. Can even assign one structure to another
- `typedef existing_type new_type`
- The most important use for `typedef` is in renaming structures and unions,
...
- For binary files you should add a "b" after the above strings.

### Macros

- In a macro value, the operator `#` may be used to expand a macro parameter to the textual appearance of the actual argument in the form of a string literal, exactly as supplied in the calling source. The parameter must follow the '#' immediately. It is mostly used for debugging or logging purposes
- In a macro value, the operator `##` may be used to concatenate a macro parameter's invocation argument with adjacent text. This can be used to generate identifiers.
- Any macro may be undefined using the `#undef` preprocessor directive.
...
- `__TIME__` compilation date (String literal)
...
- `char *args[2]` : is an array of pointer to strings. Used as `args[0] = "abcde", args[1] = "def"`
...
- There's a difference between the two constructs above. The first construct is an array of 5 integers pointed by `ptr`. Used as `ptr[0] = 0; *(ptr+1) = 1`
- The second is a an array of 5 pointers that points to ints.
