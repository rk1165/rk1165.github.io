...
- On choosing the application architecture, an application developer will likely draw on one of the two predominant architectural paradigms used in modern network applications: the client-server architecture or the P2P architecture.
...
- In the jargon of OS, it is not actually programs but **processes** that communicate.
...
- An example timing guarantee might be that every but that the sender pumps into the socket arrives at the receiver's socket no more than 100 msec later.
...
- The HTTP is defined in RFC 1945 and RFC 2616.
...
- Request line - `method|sp|URL Version|sp|cr|lf`
- Header lines - `header field name:|sp|value|cr|lf`
- Blank line - `cr|lf`
- Entity body - used with the POST method.
...
- The _Last-Modified:_ header is critical or object caching, both in the local client and the network cache servers (also known as proxy servers).
...
- another important DNS server **local DNS server** does not strictly belong to the hierarchy but central to the DNS architecture. Each ISP—residential or institutional has a local DNS server (also
  called a default name server). When a host connects to an ISP, the ISP provides the host with the IP addresses of one or more of its local DNS servers (typically through DHCP)
...
- If `Type=NS` the `Name` is a domain (such as foo.com) and `Value` is the hostname of an authoritative DNS server that knows how to obtain the IP addresses for hosts in the domain.
...
- A **registrar** is a commercial entity that verifies the uniqueness of
  the domain name, enters the domain name into the DNS database (as discussed below), and collects a small fee from you for its services.
- A more effective DDoS attack agains DNS would be to send a deluge of DNS queries to top-level-domain servers.
- DNS has four components
  ...
  - TLD Nameservers : These contains information about domains under that TLD.
...
- Dynamic Adaptive Streaming over HTTP (DASH). In DASH, the video is encoded into several different versions, with each version having a
...
- (What else can be distributed P2P)
