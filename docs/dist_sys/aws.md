...
- AZ scope : EC2
- The virtual server is known as an EC2 instance. It runs on a physical host that is inside and AWS AZ. There will be two or more AZ within an AWS Region.
- IAM : allows to create users, who can interact with ...
...
- To know more about CIDR notation [Classless Internetwork Routing](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing#CIDR_notation)
...
- vpc-029ac8721287a50a1 : vpc id
- subnet-0c0254542cc30e871 : subnet id
...
- [AWS Cloud9](https://docs.aws.amazon.com/cloud9/latest/user-guide/welcome.html) is a cloud-based IDE.
...
- You can store files as [Objects](https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingObjects.html) withing [S3 Buckets](https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingBucket.html)
...
- In order to asynchronously process the photo labels, when a photo is uploaded, an Amazon S3 bucket event notification is issued to an Amazon SNS topic. This triggers a subscribed AWS Lambda function, which talks to Amazon Rekognition
- To make the application more distributed, an Amazon SQS queue subscribed to the Amazon SNS topic stores all the incoming requests and an on-premises application polls the queue for processing
- AWS X-Ray traces the calls made to all the AWS resources in this application, thereby providing diagnostics information.
- The application is coded in Python 3 using AWS Cloud9 as the IDE.
  ![architecture](./images/arch.png)

#### Amazon Rekognition

- [Amazon Rekognition](https://docs.aws.amazon.com/rekognition/latest/dg/what-is.html) is a service that applies deep learning to analyze the contents of images and videos. It supports functionality such scene detection, face detection, even celebrity recognition.
...
- To explore Signature V4 Signing further, here's some [example code](https://docs.aws.amazon.com/general/latest/gr/sigv4-signed-request-examples.html) that creates HTTP requests and generates the appropriate signature.
...
- **CloudWatch**: It's a service allowing to monitor.
- **Route 53** is DNS. To create name records.
- **VPC** : Allows to control the networking for EC2. Site-to-site VPN from your business to AWS.
- Auto Scaling : To automatically create instances and remove them. You could define TimeFrames.
- **CloudFormation** allows to define templates of Servers. People have preconfigured template servers
- **IAM** : Is the ability to create credentials to access your system.
- **ELB** : Ability to load balance between servers
- **EBS** : Permanent storage location. (Elastic Bean Stalk)
...
- [Allow users to connect on HTTP or HTTPS](https://aws.amazon.com/premiumsupport/knowledge-center/connect-http-https-ec2/)
...
- Capacity, Performance or Availability issues can arise.
...
- [AWS RDS security](https://aws.amazon.com/rds/features/#security)
...
- Here are additional resources that you might want checkout:
...
- Security Group
  ...
  - Sources could be - iprange, subnet, other security groups
  - We define Rules which are inbound and outbound
  - Rules are stateful
...
- Allow you to create workflows that follow a fixed or dynamic squeuence - aka "steps"
