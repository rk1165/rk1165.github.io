...
- X-pack adds additional features to ES, and Kibana. Such as Security, Monitoring, Alerting, Reporting, Also enables Kibana to do ML, Graph, ES SQL
...
- When a node starts up it will either join a new cluster or starts it own
- Each unit of data is stored as document. Document is json
...
- Replication is used to ensure high availability for indices. A side benefit is increased query throuhgput
...
- How to add nodes to cluster:
  - Download & extract another ES and start it up. Just change the configurations (such as changing the node.name). This approach is recommended as each node will have it dedicated config files and data folder.
  - `./elasticsearch -Enode.name=node-2 -Epath.data=./node-2/data -Epath.logs=./node-2/logs` : this is a short cut way.
...
- When a text value is indexed a so called analyzer is used to process the text. IOW the value is analyzed.
...
- Some of the important mapping parameters
  ...
  - `properties` : defiens nested fiedls for `object` and `nested` fields
