...
- `awk '{ print $0 }' /etc/passwd` Whenever you pass the "" string to the print command, it prints a blank line.
...
- `$5 ~ /root/ { print $3 }` will print only the third field on the line if the first field on the same line contains the character sequence root
